...
<template>
  <section class="wh_container">
    <div class="begin_end">
        <div class="begin">
          开始日期
          <p>{{this.historyChose[0]}}</p>
        </div>
        <div class="end">
          结束日期
          <p>{{this.historyChose[1]}}</p>
        </div>
    </div>
    ...
    <router-link to="/home/personaltailor" class="confirm_btn">
      确认
    </router-link>
  </section>
</template>
    ...
    getList: function (date, chooseDay, isChosedDay = true) {
      ...
      for (let i = 0; i < arr.length; i++) {
        ...
        if (chooseDay && chooseDay === nowTime && flag) {
          ...
          if( this.historyChose.length>2 ){
            this.historyChose.shift();
          }
          k.chooseDay = true;
        } else if (
